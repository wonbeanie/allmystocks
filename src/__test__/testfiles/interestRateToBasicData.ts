import { stocksDataList } from "../../modules/stock/stockTypes";

export default {
    "Financial Select Sector SPDR ETF US81369Y6059" : {
        stockName : "Financial Select Sector SPDR ETF",
        stockCode : "US81369Y6059",
        stockType : "US",
        financial : {
            totalPrice : "122",
            buyFirstTime : "2023.07.05 (12:56:55)",
            interestInstallmentSaving : "2.23",
            interestDeposits : "4.11",
            totalReturn : "0",
            totalDividend : "0"
        },
        stockState : {
            totalPrice : "122",
            profit : "0",
            flatPrice : "40",
            retentionTime : "1년 2개월",
            amount : "3",
            buyFirstTime : "2023.07.05 (12:56:55)",
            currentPrice : "44.95"
        },
        stockHistory : {
            totalPricePercent : "2.98",
            totalReturn : "0",
            currentPrice : "44.95",
            history : [
                {
                    conclusionAmount : "122",
                    historyProfit : "-",
                    conclusionDate : "2023.07.05 (12:56:55)",
                    unitPrice : "40",
                    detailContext : "외화증권매수",
                    tradeType : "매수",
                    quantity : "3"
                }
            ]
        }
    },
    "iShares Expanded Tech Sector ETF US4642875490" : {
        stockName : "iShares Expanded Tech Sector ETF",
        stockCode : "US4642875490",
        stockType : "US",
        financial : {
            totalPrice : "555",
            buyFirstTime : "2023.02.15 (11:23:52)",
            interestInstallmentSaving : "4.22",
            interestDeposits : "7.78",
            totalReturn : "48",
            totalDividend : "0"
        },
        stockState : {
            totalPrice : "555",
            profit : "0",
            flatPrice : "92",
            retentionTime : "1년 6개월",
            amount : "3",
            buyFirstTime : "2023.02.15 (11:23:52)",
            currentPrice : "86.9"
        },
        stockHistory : {
            totalPricePercent : "13.57",
            totalReturn : "48",
            currentPrice : "86.9",
            history : [
                {
                    conclusionAmount : "324",
                    historyProfit : "48",
                    conclusionDate : "2023.07.04 (14:45:12)",
                    unitPrice : "108",
                    detailContext : "외화증권매도",
                    tradeType : "매도",
                    quantity : "3"
                },
                {
                    conclusionAmount : "555",
                    historyProfit : "-",
                    conclusionDate : "2023.02.15 (11:23:52)",
                    unitPrice : "92",
                    detailContext : "외화증권매수",
                    tradeType : "매수",
                    quantity : "6"
                }
            ]
        }
    },
    "Invesco NASDAQ 100 ETF US46138G6492" : {
        stockName : "Invesco NASDAQ 100 ETF",
        stockCode : "US46138G6492",
        stockType : "US",
        financial : {
            totalPrice : "1,886",
            buyFirstTime : "2023.04.24 (11:34:56)",
            interestInstallmentSaving : "34.52",
            interestDeposits : "63.55",
            totalReturn : "0",
            totalDividend : "0"
        },
        stockState : {
            totalPrice : "1,886",
            profit : "0",
            flatPrice : "186",
            retentionTime : "1년 4개월",
            amount : "10",
            buyFirstTime : "2023.04.24 (11:34:56)",
            currentPrice : "15.74"
        },
        stockHistory : {
            totalPricePercent : "46.12",
            totalReturn : "0",
            currentPrice : "15.74",
            history : [
                {
                    conclusionAmount : "353",
                    historyProfit : "-",
                    conclusionDate : "2023.07.04 (12:46:48)",
                    unitPrice : "176",
                    detailContext : "외화증권매수",
                    tradeType : "매수",
                    quantity : "2"
                },
                {
                    conclusionAmount : "955",
                    historyProfit : "-",
                    conclusionDate : "2023.04.26 (09:23:15)",
                    unitPrice : "191",
                    detailContext : "외화증권매수",
                    tradeType : "매수",
                    quantity : "5"
                },
                {
                    conclusionAmount : "578",
                    historyProfit : "-",
                    conclusionDate : "2023.04.24 (11:34:56)",
                    unitPrice : "192",
                    detailContext : "외화증권매수",
                    tradeType : "매수",
                    quantity : "3"
                }
            ]
        }
    },
    "Global X Robotcs Artificial Intellig ETF US37954Y7159" : {
        stockName : "Global X Robotcs Artificial Intellig ETF",
        stockCode : "US37954Y7159",
        stockType : "US",
        financial : {
            totalPrice : "269",
            buyFirstTime : "2023.06.29 (14:12:36)",
            interestInstallmentSaving : "4.92",
            interestDeposits : "9.06",
            totalReturn : "0",
            totalDividend : "0"
        },
        stockState : {
            totalPrice : "269",
            profit : "0",
            flatPrice : "26",
            retentionTime : "1년 2개월",
            amount : "10",
            buyFirstTime : "2023.06.29 (14:12:36)",
            currentPrice : "29"
        },
        stockHistory : {
            totalPricePercent : "6.57",
            totalReturn : "0",
            currentPrice : "29",
            history : [
                {
                    conclusionAmount : "269",
                    historyProfit : "-",
                    conclusionDate : "2023.06.29 (14:12:36)",
                    unitPrice : "26",
                    detailContext : "외화증권매수",
                    tradeType : "매수",
                    quantity : "10"
                }
            ]
        }
    },
    "동서 026960" : {
        stockName : "동서",
        stockCode : "026960",
        stockType : "KR",
        financial : {
            totalPrice : "260,980",
            buyFirstTime : "2023.04.14 (15:54:42)",
            interestInstallmentSaving : "4,778.1",
            interestDeposits : "8,795.02",
            totalReturn : "0",
            totalDividend : "0"
        },
        stockState : {
            totalPrice : "260,980",
            profit : "0",
            flatPrice : "18,680",
            retentionTime : "1년 4개월",
            amount : "14",
            buyFirstTime : "2023.04.14 (15:54:42)",
            currentPrice : "18,100"
        },
        stockHistory : {
            totalPricePercent : "4.76",
            totalReturn : "0",
            currentPrice : "18,100",
            history : [
                {
                    conclusionAmount : "185,900",
                    historyProfit : "-",
                    conclusionDate : "2023.05.16 (10:23:11)",
                    unitPrice : "18,590",
                    detailContext : "코스피매수",
                    tradeType : "매수",
                    quantity : "10"
                },
                {
                    conclusionAmount : "75,080",
                    historyProfit : "-",
                    conclusionDate : "2023.04.14 (15:54:42)",
                    unitPrice : "18,770",
                    detailContext : "코스피매수",
                    tradeType : "매수",
                    quantity : "4"
                }
            ]
        }
    },
    "삼성전자우 005935" : {
        stockName : "삼성전자우",
        stockCode : "005935",
        stockType : "KR",
        financial : {
            totalPrice : "1,421,600",
            buyFirstTime : "2022.03.17 (17:25:53)",
            interestInstallmentSaving : "49,718.11",
            interestDeposits : "91,515.72",
            totalReturn : "5,292",
            totalDividend : "5,292"
        },
        stockState : {
            totalPrice : "1,421,600",
            profit : "0",
            flatPrice : "64,250",
            retentionTime : "2년 5개월",
            amount : "22",
            buyFirstTime : "2022.03.17 (17:25:53)",
            currentPrice : "55,200"
        },
        stockHistory : {
            totalPricePercent : "25.96",
            totalReturn : "5,292",
            currentPrice : "55,200",
            history : [
                {
                    conclusionAmount : "5,292",
                    historyProfit : "-",
                    conclusionDate : "2023.07.08 (12:12:22)",
                    unitPrice : "0",
                    detailContext : "배당금",
                    tradeType : "입금",
                    quantity : "0"
                },
                {
                    conclusionAmount : "127,600",
                    historyProfit : "-",
                    conclusionDate : "2023.05.05 (12:32:41)",
                    unitPrice : "63,800",
                    detailContext : "코스피매수",
                    tradeType : "매수",
                    quantity : "2"
                },
                {
                    conclusionAmount : "1,294,000",
                    historyProfit : "-",
                    conclusionDate : "2022.03.17 (17:25:53)",
                    unitPrice : "64,700",
                    detailContext : "코스피매수",
                    tradeType : "매수",
                    quantity : "20"
                }
            ]
        }
    }
} as stocksDataList;